    ...
    try:
        with open(config_path, "r") as f:
            ...
            if config["service"] == "azure":
                if "azure_credentials_path" not in config:
                    logger.error(
                        "Config file must contain 'azure_credentials_path' key when service is azure."
                    )
                    return None
                try:
                    with open(config["azure_credentials_path"], "r") as cred_f:
                        azure_creds = json.load(cred_f)
                        config["azure_endpoint"] = azure_creds["azure_endpoint"]
                        config["azure_key"] = azure_creds["azure_key"]
                except FileNotFoundError:
                    logger.error(f"Azure credentials file not found at: {config['azure_credentials_path']}")
                    return None
                except (KeyError, json.JSONDecodeError):
                    logger.error(f"Error reading Azure credentials file. Ensure it is a valid JSON with 'azure_endpoint' and 'azure_key'.")
                    return None
